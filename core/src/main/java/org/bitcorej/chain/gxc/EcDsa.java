...
public class EcDsa {
    ...
    private static BigInteger deterministicGenerateK(CurveParam curveParam, byte[] hash, BigInteger d, SigChecker checker, int nonce) {
        if (nonce > 0) {
            hash = Sha256.from(hash, GxcPrivateKey.getSecuRandom().generateSeed(nonce)).getBytes();
        }
    ...
    public static EcSignature sign(Sha256 hash, GxcPrivateKey key) {
        ...
        GxcPublicKey pubKey = key.getPublicKey();

        for (int i = 0; i < 4; i++) {
            GxcPublicKey recovered = recoverPubKey(curveParam, data, signature, i);
...
//        String r = signature.r.toString(16);
//        String s = signature.s.toString(16);
        return signature;
    }

    public static GxcPublicKey recoverPubKey(byte[] messageSigned, EcSignature signature) {
        return recoverPubKey(signature.curveParam, messageSigned, signature, signature.recId);
    }

    public static GxcPublicKey recoverPubKey(CurveParam curveParam, byte[] messageSigned, EcSignature signature, int recId) {
        ...
        return new GxcPublicKey(q.getEncoded());
    }


    public static boolean isSignerOf(CurveParam curveParam, byte[] messageSigned, int recId, EcSignature sig, byte[] pubKeyBytes) {
        Preconditions.checkArgument(recId >= 0, "recId must be positive");
